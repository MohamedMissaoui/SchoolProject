...
Il s'appuie sur une représentation standard des objets proposée par le protocole XDR et le mécanisme d'appels de procédures distantes implémenté par le protocole RPC (Remote Procedure Call). Tous ces deux protocoles sont aussi l'oeuvre de Sun Microsystem.
...
Les commandes : La création de snapshot avec LVM se fait de façon simple : on crée un nouveau volume logique qui
contient le snapshot, on monte ce volume, on effectue sa sauvegarde, on démonte le volume puis on le
détruit.
